...
/**
 * This class implements a simple HTTP server. A AJPServer is bound to an IP address
 ...
 * to a AJPServer is encapsulated by a {@link HttpContext} object. HttpContexts
 ...
 * For example. Given a AJPServer with the following HttpContexts configured.<p>
 * <table >
 ...
 * be accepted by the AJPServer. When the limit is reached, further connections may be
...
public abstract class AJPServer extends HttpServer {
    ...
    /**
     * creates a AJPServer instance which is initially not bound to any local address/port.
     * The AJPServer is acquired from the currently installed {@link AJPServerProvider}
     * The server must be bound using {@link #bind(InetSocketAddress,int)} before it can be used.
     * @throws IOException
     */
    public static AJPServer create () throws IOException {
        return create (null, 0);
    }

    /**
     * Create a <code>AJPServer</code> instance which will bind to the
    ...
    public static AJPServer create(InetSocketAddress addr, int backlog) throws IOException {
     ...
     * @throws IOException
     */
    public static AJPServer create(InetSocketAddress addr, int backlog, Map<String, String> configuration) throws IOException {
        HttpServerProvider provider = HttpServerProvider.provider();
        return provider.createAJPServer(addr, backlog, configuration);
    }    
    
    /**
     * Binds a currently unbound AJPServer to the given address and port number.
     * A maximum backlog can also be specified. This is the maximum number of
     * queued incoming connections to allow on the listening socket.
     * Queued TCP connections exceeding this limit may be rejected by the TCP implementation.
     ...
     * Once stopped, a AJPServer cannot be re-used. <p>
     *
     ...
     * URI path to a exchange handler on this AJPServer. Once created, all requests
     ...
     * URI path to an exchange handler on this AJPServer. Once created, and when
