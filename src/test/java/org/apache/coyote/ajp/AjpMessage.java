...
public class AjpMessage {
    ...
    private static final Logger log = Logger.getLogger(AjpMessage.class.getCanonicalName());
    ...
    public void appendBytes(MessageBytes mb) {
        if (mb == null) {
            log.log(Level.SEVERE, "AJP Message cannot be null", new NullPointerException());
            appendInt(0);
            appendByte(0);
            return;
        }
    ...
    public void appendByteChunk(ByteChunk bc) {
        if (bc == null) {
            log.log(Level.SEVERE, "AJP ByteChunk cannot be null", new NullPointerException());
            appendInt(0);
            appendByte(0);
            return;
        }
    ...
    public void appendCharChunk(CharChunk cc) {
        if (cc == null) {
            log.log(Level.SEVERE, "CharChunk cannot be null", new NullPointerException());
            appendInt(0);
            appendByte(0);
            return;
        }
    ...
    public void appendString(String str) {
        if (str == null) {
            log.log(Level.SEVERE, "AJP Message was null", new NullPointerException());
            appendInt(0);
            appendByte(0);
            return;
        }
    ...
    public void appendBytes(byte[] b, int off, int numBytes) {
        if (pos + numBytes + 3 > buf.length) {
            log.log(Level.SEVERE, "ajpmessage.overflow:" + numBytes + ", Pos: " + pos, new ArrayIndexOutOfBoundsException());
    ...
    public int processHeader(boolean toContainer) {
        ...
        if ((toContainer && mark != 0x1234) ||
            ...
            log.log(Level.SEVERE, "AJP Message invalid due to invalid mark: " + mark);
            if (log.isLoggable(Level.FINE)) {
                dump("In: ");
            }
            return -1;
    ...
    public void dump(String msg) {
        if (log.isLoggable(Level.FINE)) {
            log.log(Level.FINE, msg + ": " + HexUtils.toHexString(buf) + " " + pos +"/" + (len + 4));
        }
        int max = pos;
        if (len + 4 > pos)
            max = len+4;
        ...
        if (log.isLoggable(Level.FINE)) {
            for (int j = 0; j < max; j += 16) { 
                log.log(Level.FINE, hexLine(buf, j, len));
            }
        }
    }
    ...
    private void validatePos(int posToTest) {
        if (posToTest > len + 4) {
            ...
            throw new ArrayIndexOutOfBoundsException("Invalid position: " + posToTest);
        }
    }
    // ------------------------------------------------------ Protected Methods
